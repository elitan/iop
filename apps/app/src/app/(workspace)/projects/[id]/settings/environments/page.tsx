...
export default function EnvironmentsSettingsPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { envId: string; name: string }) =>
  ...
  const deleteMutation = useMutation({
    mutationFn: (envId: string) => client.environments.delete({ envId }),
    onSuccess: async () => {
      await queryClient.refetchQueries({
        queryKey: orpc.environments.list.queryOptions({ input: { projectId } })
          .queryKey,
  ...
  function handleRename() {
    ...
    updateMutation.mutate({ envId: editingEnv.id, name: newName.trim() });
  }
